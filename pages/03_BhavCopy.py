...
st.set_page_config(page_title="BhavCopy Downloader", page_icon=":bar_chart:", layout="wide",initial_sidebar_state="expanded",)
#st.markdown("### Site is in progress \n Shall be launched asap")
st.title("BHAVCOPY especially for AMIBROKER USERS")
...
with col2:
    if my1_date is not date.today():
        my2_date = st.date_input("TILL", value=min(my1_date+timedelta(90),date.today()),
        min_value=datetime.date(1990, 1, 1))
    else :
...
if st.button("DOWNLOAD"):
    try:
        EOD.download_bhav1(my1_date,my2_date)
